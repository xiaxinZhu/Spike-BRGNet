...
class FullModel(nn.Module):
  ...
  def forward(self, inputs, labels, bd_gt, *args, **kwargs):
    ...
    # loss_s： extra P_branch CE loss L0(standard) + predicted CE loss L1(OHEM)
    # loss_b：weighted binary cross entropy loss
    # loss_ba：boundary-awareness CE loss L2
    ...
    loss_ba = self.sem_loss(outputs[-2], bd_label) # loss_sb:1
    # loss = loss_s + loss_b + loss_ba # loss:[b,h,w]
    loss = loss_s + loss_ba # loss:[b,h,w]
    # size: loss=[1,b,h,w], outputs=[x_extra_p, x_], acc=1, [loss_s, loss_b]
    return torch.unsqueeze(loss,0), outputs[:-1], acc, [loss_s, loss_ba]
